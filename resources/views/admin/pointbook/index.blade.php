...
@section('title','Point Book') @section('page-title','Point Book Kegiatan')
@section('content')
<div class="nim-search-box mb-3">
    <div class="d-flex gap-2 align-items-end flex-wrap">
        ...
        @if(request('nim'))<a href="{{ route('admin.pointbook.index') }}" class="btn btn-light">Reset</a>@endif
    </div>
    @if(request('nim') && $rekapNim !== null)
    <div class="mt-2">
        <div class="alert alert-light py-2 mb-0 d-inline-flex align-items-center gap-2">
            <i class="fas fa-medal text-warning fa-lg"></i>
            <span>Total Poin untuk NIM <strong>{{ request('nim') }}</strong>: 
            <span class="badge bg-warning text-dark fs-6">{{ $rekapNim }} Poin</span></span>
        </div>
    </div>
    @endif
</div>
<div class="row g-3">
    <div class="col-lg-4">
        <div class="card">
            ...
            <div class="card-body">
                <form method="POST" action="{{ route('admin.pointbook.store') }}">
                    @csrf
                    <div class="mb-2"><label class="form-label fw-semibold">NIM <span class="text-danger">*</span></label>
                        <div class="d-flex gap-1">
                            <input type="text" name="nim" id="nimPoint" class="form-control" value="{{ old('nim', request('nim')) }}" required placeholder="22.240.0001">
                    ...
                    <div class="mb-2"><label class="form-label fw-semibold">Nama Kegiatan <span class="text-danger">*</span></label>
                        <input type="text" name="nama_kegiatan" class="form-control" value="{{ old('nama_kegiatan') }}" required placeholder="Lomba, Seminar, dll..."></div>
                    <div class="row g-2 mb-2">
                        <div class="col-6"><label class="form-label fw-semibold">Poin <span class="text-danger">*</span></label>
                            <input type="number" name="poin" class="form-control" value="{{ old('poin',1) }}" min="1" required></div>
    ...
    <div class="col-lg-8">
        <div class="card">
            <div class="card-header"><i class="fas fa-medal me-2"></i>Data Point Book ({{ $points->total() }})</div>
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-hover">
                        ...
                        <tr>
                            ...
                            <td><small>{{ $p->mahasiswa?->nama }}</small></td>
                            <td>{{ $p->nama_kegiatan }}</td>
                            <td><span class="badge bg-warning text-dark fs-6"><i class="fas fa-star me-1"></i>{{ $p->poin }}</span></td>
                            <td><small>{{ \Carbon\Carbon::parse($p->tanggal)->format('d/m/Y') }}</small></td>
                            <td><small class="text-muted">{{ $p->keterangan ?? '-' }}</small></td>
                            <td>
                                <form action="{{ route('admin.pointbook.destroy',$p->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Hapus poin?')">
                                    @csrf @method('DELETE')
...
function cariNIM() {
    ...
    fetch('{{ route("admin.pointbook.search-mhs") }}?nim='+encodeURIComponent(nim), {
